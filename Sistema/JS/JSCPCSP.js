...
$( document ).ready( function ()
{
    ...
    $( "#diag-load, #diag-error, #diag-ok, #dMje, #tInfoAcre, #tRepoIrreg, #dSS, #tNomInfor" ).hide();
...
function guardaDenuncia()
{
    ...
    try
    {
        ...
        if ( $( "#txtTema" ).val() == "" )
        {
            MensajeError( "Debe capturar el tema, acción o programa respecto de la cual se presenta la queja, denuncia o sugerencia." );
            $( "#txtTema" ).css( "border-color", "red" );
            Recaptcha.reload();
            return false;
        }

        if ( $( "#txtComoAfecta" ).val() == "" )
        {
            ...
            $( "#txtComoAfecta" ).css( "border-color", "red" );
            Recaptcha.reload();
            return false;
        }
        else if ( $( "#txtComoAfecta" ).length > 3999 )
        {
        ...
        $( "#txtTema" ).css( "border-color", "" );
        $( "#txtComoAfecta" ).css( "border-color", "" );
        ...
        $.ajax( {
            type: "POST",
            url: "CPCSP.aspx/AJAX_verificaReCaptcha",
            data: _oData,
        ...
        .done( function ( data, textStatus, jqXHR )
        {
            ...
            if ( _aResp[1] == "True" )
            {
                ...
                _oData = "{ _psNombreInform: '" + $( "#txtNombreDen" ).val() + "' " +
                ...
                $.ajax( {
                    type: "POST",
                    url: "CPCSP.aspx/AJAX_guardaDenuncia",
                    data: _oData,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json"
                } )
