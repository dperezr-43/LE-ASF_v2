...
$( document ).ready( function ()
{
    ...
    //$( "#diag-load, #diag-error, #diag-ok, #dMje, #tAfecta, #tInfoAcre, #tRepoIrreg, #dSS" ).hide();
    ...
    $( "#diag-load, #diag-error, #diag-ok, #dMje, #tAfecta, #tInfoAcre, #tRepoIrreg, #dSS, #tPF, #tNomInfor" ).hide();

    $( "#rdPFSi" ).change( function ()
    {
        if ( $( this ).prop( "checked" ) )
        {
            $( "#tPF" ).show();
...
function validaPrimSecc()
{
    try
    {
        ...
        if ( $( "#txtNombCE" ).val() == "" )
        {
            MensajeError( "Debe capturar el Nombre y Clave del Centro Escolar al que pertenece el informante." );
            $( "#txtNombCE" ).css( "border-color", "red" );
            Recaptcha.reload();
            return false;
        }

        $( "#txtNombCE" ).css( "border-color", "" );

        //if ( $( "#txtDirDen" ).val() == "" )
        //{
        //    MensajeError( "Debe capturar el domicilio o correo electrónico para recibir información." );
...
function guardaDenuncia()
{
    ...
    try
    {
        ...
        if ( $( "#rdPFSi" ).prop( "checked" ) && $( "#txtNombProg" ).val() == "" )
        {
            MensajeError( "Si la queja, denuncia, o sugerencia es sobre algún programa federal, estatal o municipal que se aplica en el centro escolar debe de detallar el nombre del programa." );
            $( "#txtNombProg" ).css( "border-color", "red" );
            Recaptcha.reload();
            return false;
        }
        else if ( $( "#rdPFSi" ).prop( "checked" ) && $( "#txtNombProg" ).length > 3999 )
        {
            MensajeError( "El detalle del Programa no debe ser mayor a 4000 carateres." );
            $( "#txtNombProg" ).css( "border-color", "red" );
            Recaptcha.reload();
            return false;
        }
        ...
        {
            MensajeError( "Si la queja, denuncia, o sugerencia afecta la terminación, conservación u operación de la obra debe detallar como afecta." );
        ...
        $.ajax( {
            type: "POST",
            url: "CEPS.aspx/AJAX_verificaReCaptcha",
            data: _oData,
        ...
        .done( function ( data, textStatus, jqXHR )
        {
            ...
            if ( _aResp[1] == "True" )
            {
                ...
                _oData = "{ _psNombreCentro: '" + $( "#txtNombCE" ).val() + "' " +
                            ...
                            ", _psQuejaNombC: '" + $( "#txtNombProg" ).val() + "' " +
                            ", _psAfectaTerm: '" + $( "#txtAfecta" ).val() + "' " +
                ...
                $.ajax( {
                    type: "POST",
                    url: "CEPS.aspx/AJAX_guardaDenuncia",
                    data: _oData,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json"
                } )
...
function cargaMunicipios( _plLlaveEnt )
{
    try
    {
        ...
        $.ajax( {
            type: "POST",
            url: "CEPS.aspx/AJAX_traeMunicipios",
            data: _oData,
            contentType: "application/json; charset=utf-8",
            dataType: "json"
        } )
